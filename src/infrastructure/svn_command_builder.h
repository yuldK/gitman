...
namespace gitman {
    ...
    [[nodiscard]] process_request make_svn_info_item_request(std::u8string_view executable, std::u8string_view working_directory, svn_info_item item, std::u8string_view target = {}, const vcs_timeout_overrides& timeouts = {});

    // URL을 대상으로 값 하나를 조회한다. 원격을 실제로 확인하므로 원격 조회 한도를 쓴다.
    [[nodiscard]] process_request make_svn_remote_info_item_request(std::u8string_view executable, std::u8string_view working_directory, svn_info_item item, std::u8string_view url, const vcs_timeout_overrides& timeouts = {});
    ...
    // 허용 목록에서 고른 URL로 전환한다. `--ignore-ancestry`, `--force`와 `--accept`는
