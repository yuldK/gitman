...
- 형식은 문서와 같은 규칙이다: UTF-8, 공백 4칸, `schema_version`, 알 수 없는
  키 보존, 원자적 교체. **backup 파일은 만들지 않는다** (2026-08-21 사용자 지시로
  문서 저장의 `.bak`도 함께 제거했다).
...
- 창이 좁으면(폭 < 720 논리 픽셀) 두 열을 세로로 쌓는다. 남은 높이에 목록이
  다 들어가지 않으면 **들어가는 행만 그리고 마지막 줄에 "외 N개 더 있습니다"**
  로 알린다 (구현 결정: 항목이 최대 10개라 스크롤 상태를 새로 두지 않았다).
- 시작 페이지에서는 하단 로그 pane을 두지 않는다. 문서가 없으면 로그도 없고,
  pane이 차지하던 160 논리 픽셀을 시작 페이지가 쓴다.
...
- 확장: `view_snapshot.h`, `app_messages.h`(intent 2종), `logic_controller`,
  ...
  (MessageBox 호출 2곳 → intent 게시), `ui_interaction`의 Esc 처리(알림이 가장
  먼저 닫힌다).

---

## A5. 문서 닫기와 생성 아이콘 (2026-08-21 추가 지시)

> 프로젝트가 열린 경우에 명시적으로 닫을 수 있으면 좋겠다. 열려 있는 상태에서
> 새 프로젝트 만들기 아이콘이 안 보였으면 좋겠다.

### A5.1 결정

- toolbar에 **문서 닫기 버튼**(`$(close)`)을 둔다. 문서가 열려 있을 때만 보이며
  누르면 `close_document_intent`가 나간다.
- **새 문서 만들기 버튼은 문서가 없을 때만** 둔다. 열린 상태에서 새 문서를
  만들려면 먼저 닫는다. 문서가 없을 때의 진입점은 toolbar와 시작 페이지 두
  곳이다(A1.3).
- 문서 열기 버튼은 지금처럼 문서가 없을 때만 보인다. 결과적으로 toolbar는
  "문서 없음 = 열기·만들기", "문서 있음 = 닫기"로 대칭이 된다.
- **배치**(2026-08-21 추가 지시): 전체 새로 고침은 **왼쪽 끝**, 문서 닫기는
  **오른쪽 끝**이다. 문서 경로는 새로 고침 오른쪽에서 시작하고, 나머지 버튼
  (환경설정·탐색·경로 표시·열기·만들기)은 닫기 왼쪽으로 이어진다. 창이 좁으면
  지금처럼 오른쪽 버튼부터 자리가 없는 순서로 숨는다.

### A5.2 닫기가 하는 일

문서 열기의 상태 정리와 같은 규칙을 쓴다.

1. 진행 중인 변경 작업을 취소한다(카드가 사라지기 전에 전파).
2. 문서에 반영되지 않은 창 배치가 있으면 저장을 한 번 내보낸다. 이미 제출된
   저장은 worker가 끝까지 수행하고 늦은 결과는 operation id 비교로 버려진다.
3. 문서·카드·선택·필터·스크롤·로그 뷰 상태와 이전 문서를 가리키던 dialog·
   메뉴를 모두 버린다. 탐색이 진행 중이면 취소한다.
4. 화면은 `no_document`가 되어 시작 페이지로 돌아간다. **최근 목록은 그대로**
   두므로 방금 닫은 문서가 목록 맨 앞에 남는다.

확인 창은 두지 않는다. 문서 변경은 항상 즉시 저장되므로 닫기로 잃는 것이 없다.
