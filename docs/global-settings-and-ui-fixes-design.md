...
- **문서가 열려 있으면 문서 override를 편집한다.** 제목은 `환경설정 (문서)`.
  ...
  - 사용자가 어떤 행의 값을 바꾸면 그 행이 "문서에 정의됨"이 된다. 저장 시
    ...
    따른다.
  - 문서에 정의되지 않은 행은 값 옆에 `앱 설정 따름` 문구를 흐리게 보여 준다.
  - 실행 파일 행의 `지우기`는 모드에 따라 다르다: 전역 모드에서는 빈 값(자동
    탐색), 문서 모드에서는 **문서 정의 제거**(앱 설정 따름으로 복귀)다. 토글·
    제한 시간 행은 되돌리기 컨트롤을 두지 않는다(필요해지면 후속).
