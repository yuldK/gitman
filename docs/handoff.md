...
- 현재 체크포인트: `S3-D4-CODE` 구현 완료, 사용자 검수 대기 (`S3-P0`와 `S3-D1`~`S3-D3` 구간 승인 및 FIX 생략 완료)
- 다음 허용 작업: 승인 후 `S3-D4-TEST` 하나만 수행하고 다시 보고
...
- 현재 검증 기록: `docs/verification/2026-08-16-stage-3-d4-code.md`
- 직전 검증 기록: `docs/verification/2026-08-16-stage-3-d3-test.md`
...
| --- | --- |
| 계획 ID | `S3-D4-CODE` |
| 제출 내용 | 사전 취소 확인, kill-on-close job, 정지 시작과 job 배정, 취소 event 대기, timeout과 트리 종료 |
| production code | `platform/win32/win32_process_runner.cpp` |
| test code 및 fixture | 이번 구간 변경 없음. 도우미의 `sleep`과 `spawn-child`는 `S3-D4-TEST`에서 추가한다. |
| 검증 | VS2022 Debug/Release와 VS2026 Debug 전체 CTest 각각 118/118, VS2022 `/analyze` 무경고, aggregate format/style, `git diff --check` 통과 |
| 수동 확인 | 임시 프로그램으로 19개 항목 통과 후 삭제. 손자 종료와 30회 반복 후 handle 미증가를 포함한다. |
| 발견 결함 | production 결함 후보 없음 |
| 승인 대기 | `S3-D4-CODE` 검수 |
| 승인 뒤 다음 작업 | `S3-D4-TEST` 도우미 명령 추가와 timeout 및 취소 test만 허용 |
...
| `S3-D4-CODE` timeout과 취소 production code | 검수 대기 | `docs/verification/2026-08-16-stage-3-d4-code.md` |
| `S3-D4-TEST` ~ `S3-D5-FIX` | 시작 전 | 계획 7장 순서대로 하나씩 진행 |
