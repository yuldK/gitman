...
## 2026-08-22 - 테마 탭 이름 · 초안 미리 보기 · 배지 위치 재조정

### 사용자 지시

- `외양` 탭 이름을 `테마`로 바꾼다.
- 테마에서 값을 고르면 즉시 보여 주되, 취소하면 되돌린다.
- 배지가 오른쪽 끝에 붙어 한 줄의 조작 요소가 모두 밀린다. 세부 설정 바로
  옆(오른쪽)이나 바로 위에 붙인다.

### 반영 내용

- **탭 이름**: 표시 문자열만 `테마`로 바꿨다. 열거자와 element owner 값은 저장·
  정체성에 쓰여 그대로 둔다.
- **초안 미리 보기**: dialog가 열려 있는 동안에는 테마·키 컬러 초안을 화면에
  그대로 그린다. `저장`이 값을 남기고, `취소`는 초안을 버려 다음 snapshot부터
  원래 색으로 돌아간다(별도 롤백 코드가 없다). 파일은 `저장` 전까지 그대로다.
- **배지 위치**: 항목 제목 **바로 위 왼쪽**으로 옮기고 행 컨트롤은 제목 줄
  오른쪽 끝을 그대로 쓴다. 제목 바로 오른쪽은 배치가 글꼴 없는 logic thread에서
  계산되어(`build_ui_tree`) 제목 폭을 알 수 없어 성립하지 않아, 지시의 두 번째
  안을 골랐다. 덮어쓸 수 있는 항목만 배지 줄(20px)을 갖고 전역 모드에는 없다.
- 배지·미리 보기 관련 test 5 case를 새 규칙으로 고쳐 썼다.

### 영향 요구사항

- REQ-017 (환경설정)

## 2026-08-22 - 범위 배지 · 외양 초안 · 색 격자 · 문서 닫기 배치 복원

### 사용자 지시

- 문서를 닫을 때 앱 단위 설정에 기록된 창 크기·위치로 복구한다.
- `덮어씀` 때문에 항목 왼쪽이 비는 것이 보기 나쁘다. 배지를 오른쪽에 두고,
  덮어쓰지 않은 항목은 비활성 색 `전역 설정`으로 표시한다. `덮어씀`은
  `문서 설정`으로 바꾼다.
- 색 목록을 되도록 정사각형에 가깝게, 비슷한 색끼리 한 줄에 오도록 배치한다.
- 테마가 `저장`을 누르지 않아도 적용되는 문제를 고친다.
- (진행 방식) 이전과 같이 브랜치에서 커밋까지 자동 진행하고 마지막에 보고한다.

### 반영 내용

- **문서 닫기**: 문서를 닫으면 앱 설정의 배치를 다시 적용한다. 문서를 열 때 문서
  배치를 적용하는 것과 같은 게시 번호 경로를 쓴다.
- **외양 초안화**: 테마·키 컬러를 클릭 즉시 반영하던 것을 다른 항목과 같은
  `저장`/`취소` 초안으로 바꿨다(2026-08-22 T3.3 결정을 뒤집는다). 클릭은 dialog
  초안만 고치고, `저장`이 문서 override 또는 앱 설정에 반영한다. dialog가 닫힌
  뒤 도착한 클릭은 버린다.
- **범위 배지**: 문서 모드에서 덮어쓸 수 있는 모든 항목이 제목 줄 오른쪽 끝에
  배지를 갖는다. `문서 설정`은 강조색이고 누르면 정의를 지우며, `전역 설정`은
  비활성 색이고 액션 없이 tooltip만 있다. 행 컨트롤(찾아보기·토글·세그먼트)은
  배지 열만큼 왼쪽으로 밀린다.
- **색 격자**: 한 줄에 `ceil(sqrt(개수))`개를 담아 25색이 5 × 5가 된다. 줄
  묶음은 목록 순서가 정하므로 색상환 순서인 `assets/accents.json`이 한 줄에 한
  계열을 모은다. 동그라미는 22 → 24로 키웠다. panel 높이는 600 × 428이 된다.
- 신규 3 case를 더하고 외양·배지 관련 6 case를 새 규칙으로 고쳐 썼다.
