...
`id`는 문서 안에서 고유해야 합니다. 상대 경로는 `.version-list` 문서가 있는 폴더를 기준으로 해석합니다. `display_name`, `vcs_hint`, `settings`는 생략할 수 있으며, `vcs_hint`는 `auto`, `git`, `svn` 중 하나입니다. SVN 전환 후보를 제한하려면 프로젝트에 `svn_switch_targets` 배열을 추가합니다.
...
문서를 열면 먼저 로컬 상태를 조회합니다. 전체 새로 고침 버튼이나 `F5`를 누르면 원격 상태까지 확인합니다. 카드의 새로 고침 버튼은 해당 저장소만 조회하며, 카드를 선택한 뒤 `Enter`를 눌러도 같은 동작을 실행합니다.
...
- SVN 전환은 문서의 `svn_switch_targets`에 있는 허용 URL만 사용할 수 있습니다.
- dirty 작업 트리, 이미 사용 중인 대상과 대상 불일치는 실행 전에 차단합니다. 외부 명령은 비대화형으로 실행해 인증 프롬프트 없이 실패를 로그에 남깁니다.

### 7. 로그와 환경설정

카드를 선택하면 하단에 해당 카드의 변경 로그가 표시됩니다. 로그 창에는 전체·출력·오류 스트림 필터, 자동 스크롤, 복사, 지우기, 스크롤 막대가 있으며 연속 progress 출력은 화면에서 접혀 표시됩니다. 원본 로그는 카드당 최대 1,000개 record의 메모리 ring buffer에만 보존하며 파일 로그는 만들지 않습니다.
...
두 작업은 환경설정 창의 `연결 등록`·`연결 해제` 버튼으로도 수행할 수 있습니다. 실행 파일을 다른 위치로 옮겼다면 연결을 다시 등록해야 새 경로가 반영됩니다. 등록은 `HKCU\Software\Classes`에만 적용되며, 다른 프로그램이 선택한 사용자 연결은 제거하지 않습니다.
...
- 저장 직전 백업: 같은 폴더의 `<이름>.version-list.bak`
- 작업 로그: 메모리 전용, 카드당 1,000 record
- 파일 연결: 등록한 경우 `HKCU\Software\Classes`

업그레이드는 새 `gitman.exe`로 기존 파일을 교체하면 됩니다. 실행 파일 경로가 바뀌면 파일 연결을 다시 등록합니다. 문서 저장은 임시 파일과 원자적 교체를 사용하고, 동시에 다른 프로그램이 문서를 수정하면 revision 충돌로 덮어쓰기를 막습니다. 자동 복구는 하지 않으므로 유효한 `.bak` 파일을 명시적으로 열어 복구합니다.
...
- 로그는 파일로 저장되지 않으며, 정렬 전환 UI와 Page Up/Page Down은 제공하지 않습니다.
