...
환경설정 창은 왼쪽 탭으로 나뉩니다. `도구`는 Git·SVN 실행 파일 경로와 상태 확인 제한 시간, `작업`은 submodule 갱신·로컬 변경 무시·파일 로그, `외양`은 테마와 키 컬러, `시스템`은 `.version-list` 파일 연결입니다. 저장하면 설정을 문서에 원자적으로 기록하고 도구를 다시 탐색한 뒤 카드 전체를 새로 고칩니다.

문서를 연 상태에서 값을 고치면 그 항목만 문서에 남고, 나머지는 앱 전역 설정을 따릅니다. 각 항목의 오른쪽 끝에 범위 배지가 붙어, 문서가 덮어쓴 항목은 `문서 설정`, 전역을 따르는 항목은 흐린 `전역 설정`으로 표시합니다. `문서 설정` 배지를 누르면 문서의 정의를 지우고 다시 전역 설정을 따릅니다. 테마와 키 컬러도 같은 규칙이며, 다른 항목과 마찬가지로 `저장`을 눌러야 적용됩니다. `취소`로 닫으면 고르던 값은 버려집니다.
