...
문서를 연 상태에서 값을 고치면 그 항목만 문서에 남고, 나머지는 앱 전역 설정을 따릅니다. 문서가 덮어쓴 항목은 제목 옆에 `덮어씀` 배지가 붙으며, 배지를 누르면 문서의 정의를 지우고 다시 전역 설정을 따릅니다. 테마와 키 컬러도 같은 규칙이며, 두 항목은 저장 버튼을 기다리지 않고 누르는 즉시 적용·저장됩니다.

창 크기와 위치는 문서를 열지 않았을 때만 앱 설정에 기록합니다. 문서를 열면 그때부터는 그 문서의 배치를 따르고 문서에만 저장하며, 문서를 여는 순간의 배치가 앱 설정에 남아 다음에 문서 없이 시작할 때 복원됩니다.
