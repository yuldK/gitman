...
- **환경설정 dialog**:
  ...
  - 문서 있음 → 문서 모드(제목 `환경설정 (문서)`), **암묵 덮어쓰기**(2026-08-22
    검수 결정): 값을 건드린 행만 "문서에 정의됨"이 되고, 정의되지 않은 행은
    유효 값 옆에 `앱 설정 따름`을 흐리게 보인다. 확인 시 정의된 행만 문서
    JSON에 남는다.
  - `지우기`: 전역 모드는 빈 값(자동 탐색), 문서 모드는 정의를 거둬 앱 설정을
    따른다(tooltip으로 구분). 제한 시간은 빈 칸이면 "따로 정하지 않음"이다.
...
| dialog: 전역 모드 열기·저장 (신규), 암묵 덮어쓰기·따름 표시 (신규), 문서 지우기 = 따름 복귀 (신규) | 통과 |
| 유효 설정 합성 재조회 요청 검증 (신규 test 내 포함) | 통과 |
| **전체 CTest 대응 실행: 703 case 통과 (12,306 assertions)** — 타입 변경이 전 계층에 닿아 전체를 실행했다 | 통과 |
