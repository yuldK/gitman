            ...
            LRESULT window_procedure(const UINT message, const WPARAM word_parameter, const LPARAM long_parameter)
            {
                ...
                case WM_MOUSEWHEEL:
                    if (runtime_ != nullptr)
                    {
                        ...
                        const float wheel_delta { static_cast<float>(GET_WHEEL_DELTA_WPARAM(word_parameter)) };
                        const ui::mouse_wheel_event wheel { static_cast<float>(client_point.x), static_cast<float>(client_point.y), wheel_delta, std::chrono::steady_clock::now() };
                        runtime_->post_raw_input(wheel);
                        return 0;
                    }
                    break;
                case WM_KEYDOWN:
